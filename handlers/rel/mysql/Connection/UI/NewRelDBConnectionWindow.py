...
class NewRelDBConnectionWindow(QDialog):
    def __init__(self, parent):
        ...
        # METHOD
        self.method_i = QComboBox(self)
        self.method_i.addItems(["Standard TCP / IP"])
        # self.method_i.currentIndexChanged.connect(lambda text: setattr(self.connection, 'method', text))
        self.first.addWidget(QLabel("Connection method : ", self), 2, 0, Qt.AlignRight)
        self.first.addWidget(self.method_i, 2, 1)
        self.first.addWidget(DescriptionLabel("Method to use to connect to RDBMS", self), 2, 3)
        ...
        self.hostname_i.textEdited.connect(
            lambda text: [setattr(self.connection, 'hostname', text), self._update_buttons()])
        ...
        self.port_i.textEdited.connect(
            lambda text: [setattr(self.connection, 'port', text), self._update_buttons()])
        ...
        self.parametersGridLayout.addWidget(QLabel("Username : ", self), 2, 0, Qt.AlignRight)
        self.parametersGridLayout.addWidget(self.user_i, 2, 1)
        self.parametersGridLayout.addWidget(DescriptionLabel("Name of the user to connect with : ", self), 2, 3)

        # SCHEMA
        self.schema_i = QLineEdit(self)
        self.schema_i.textEdited.connect(
            lambda text: [setattr(self.connection, 'schema', text), self._update_buttons()])
        # self.schema_i.editingFinished.connect(self._update_buttons)
        self.parametersGridLayout.addWidget(QLabel("Default schema : ", self), 4, 0, Qt.AlignRight)
        self.parametersGridLayout.addWidget(self.schema_i, 4, 1)
        self.parametersGridLayout.addWidget(DescriptionLabel("The schema to use as default schema", self), 4, 3)
    ...
    def _set_conn_params(self, conn: MySqlConnection):
        ...
        self.schema_i.setText(conn.schema)
    ...
    def connect(self):
        if self.connection.make_pool_party(self, "newConn"):
